...
public class CategoriesController : ControllerBase
{
    ...
    {
        var category = await _context.Categories.FindAsync(id); //1
        if (category == null) return NotFound();
        return Ok(new CategoryDto { Id = category.Id, Name = category.Name });
    }

    [HttpPost]
    public async Task<IActionResult> Create(CreateCategoryDto dto)  //dto.name = Dress
    {
        var category = new Category { Name = dto.Name };  //Dress
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, UpdateCategoryDto dto) //8 , dto.name = Furniture
    {
        var category = await _context.Categories.FindAsync(id);
        if (category == null) return NotFound();
        category.Name = dto.Name;
        await _context.SaveChangesAsync();
        return Ok(new { message = $"Category with id {id} {category.Name} with {dto.Name} updated successfully" });
    }
